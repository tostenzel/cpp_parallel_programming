...
template<class V>
void dmv(size_t rows, size_t cols, const V* A, const V* x, V* b, size_t num_threads, size_t chunk_size = 64/sizeof(V)) {
    auto block_cyclic = [=](size_t i) {
        size_t offset = chunk_size * i;
        size_t stride = chunk_size * num_threads;

        for (size_t lower = offset; lower < rows; lower += stride) {
            size_t upper = std::min(lower + chunk_size, rows);
    ...
    for (size_t i = 0; i < num_threads; ++i) threads.emplace_back(block_cyclic, i);
